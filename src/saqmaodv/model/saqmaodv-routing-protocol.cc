 ...
 *
 * Based on
 *      NS-2 SAQMAODV model developed by the CMU/MONARCH group and optimized and
 *      tuned by Samir Das and Mahesh Marina, University of Cincinnati;
 *
 *      SAQMAODV-UU implementation by Erik Nordström of Uppsala University
 *      https://web.archive.org/web/20100527072022/http://core.it.uu.se/core/index.php/SAQMAODV-UU
 *
...
#include "saqmaodv-routing-protocol.h"
...
NS_LOG_COMPONENT_DEFINE("SaqmaodvRoutingProtocol");

namespace saqmaodv
{
NS_OBJECT_ENSURE_REGISTERED(RoutingProtocol);

/// UDP Port for SAQMAODV control traffic
const uint32_t RoutingProtocol::SAQMAODV_PORT = 654;

/**
 * \ingroup saqmaodv
 * \brief Tag used by SAQMAODV implementation
 */
class DeferredRouteOutputTag : public Tag
{
  public:
    ...
    {
        static TypeId tid = TypeId("ns3::saqmaodv::DeferredRouteOutputTag")
                                .SetParent<Tag>()
...
{
    static TypeId tid =
        TypeId("ns3::saqmaodv::RoutingProtocol")
            ...
            .AddAttribute("UniformRv",
                  ...
                  .AddAttribute ("UseHybrid", "H-SAQMAODV hybrid mode",
                                 BooleanValue (false),
                                 MakeBooleanAccessor (&RoutingProtocol::SetUseHybrid,
                                                      &RoutingProtocol::GetUseHybrid),
                                 MakeBooleanChecker ())
                  .AddAttribute ("TVIHigh", "TVI upper threshold",
                                 DoubleValue (3.0),
                                 MakeDoubleAccessor (&RoutingProtocol::SetTVIHigh),
                                 MakeDoubleChecker<double> ())
                  .AddAttribute ("TVILow", "TVI lower threshold",
                                 DoubleValue (1.0),
                                 MakeDoubleAccessor (&RoutingProtocol::SetTVILow),
                                 MakeDoubleChecker<double> ())
                  .AddAttribute ("SigmoidTheta", "Sigmoid theta",
                                 DoubleValue (0.30),
                                 MakeDoubleAccessor (&RoutingProtocol::SetSigTheta),
                                 MakeDoubleChecker<double> ())
                  .AddAttribute ("SigmoidSigma", "Sigmoid sigma",
                                 DoubleValue (0.08),
                                 MakeDoubleAccessor (&RoutingProtocol::SetSigSigma),
                                 MakeDoubleChecker<double> ());
    return tid;
}
...
RoutingProtocol::PrintRoutingTable(Ptr<OutputStreamWrapper> stream, Time::Unit unit) const
{
    *stream->GetStream() << "Node: " << m_ipv4->GetObject<Node>()->GetId()
                         ...
                         << ", SAQMAODV Routing table" << std::endl;
                             ...
                             Socket::SocketErrno& sockerr)
{
    ...
    if (m_socketAddresses.empty())
    {
        ...
        NS_LOG_LOGIC("No saqmaodv interfaces");
        Ptr<Ipv4Route> route;
        return route;
    }
    ...
    if (m_routingTable.LookupValidRoute(dst, rt))
    {
        ...
        m_qtable.SelectHybridRoute(rt, chosenRt, &m_routingTable);
                            ...
                            const ErrorCallback& ecb)
{
    ...
    // SAQMAODV is not a multicast routing protocol
    if (dst.IsMulticast())
    {
        return false;
    }
        ...
        if (m_ipv4->GetInterfaceForAddress(iface.GetLocal()) == iif)
        {
            if (dst == iface.GetBroadcast() || dst.IsBroadcast())
            {
                ...
                if (header.GetProtocol() == UdpL4Protocol::PROT_NUMBER)
                {
                    ...
                    if (udpHeader.GetDestinationPort() == SAQMAODV_PORT)
                    {
                        // SAQMAODV packets sent in broadcast are already managed
                        return true;
                    }
                }
                if (header.GetTtl() > 1)
...
RoutingProtocol::NotifyInterfaceUp(uint32_t i)
{
    ...
    {
        NS_LOG_WARN("SAQMAODV does not work with more then one address per each interface.");
    }
    ...
    NS_ASSERT(socket);
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvSaqmaodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetLocal(), SAQMAODV_PORT));
    ...
    socket->SetRecvCallback(MakeCallback(&RoutingProtocol::RecvSaqmaodv, this));
    socket->BindToNetDevice(l3->GetNetDevice(i));
    socket->Bind(InetSocketAddress(iface.GetBroadcast(), SAQMAODV_PORT));
    socket->SetAllowBroadcast(true);
    socket->SetIpRecvTtl(true);
...
RoutingProtocol::NotifyAddAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("SAQMAODV does not work with more then one address per each interface. Ignore "
                     "added address");
    }
}
...
RoutingProtocol::NotifyRemoveAddress(uint32_t i, Ipv4InterfaceAddress address)
{
    ...
    {
        NS_LOG_LOGIC("Remove address not participating in SAQMAODV operation");
    }
}
...
RoutingProtocol::LoopbackRoute(const Ipv4Header& hdr, Ptr<NetDevice> oif) const
{
    ...
    // returned when SAQMAODV does not have a route; this causes the packet
    // to be looped back and handled (cached) in RouteInput() method
    // while a route is found. However, connection-oriented protocols
    // like TCP need to create an endpoint four-tuple (src, src port,
    // dst, dst port) and create a pseudo-header for checksumming.  So,
    // SAQMAODV needs to guess correctly what the eventual source address
    // will be.
    //
    // For single interface, single address nodes, this is not a problem.
    // When there are possibly multiple outgoing interfaces, the policy
    // implemented here is to pick the first available SAQMAODV interface.
    ...
    NS_ASSERT_MSG(rt->GetSource() != Ipv4Address(), "Valid SAQMAODV source address not found");
...
RoutingProtocol::SendRequest(Ipv4Address dst)
{
    ...
    // Send RREQ as subnet directed broadcast from each interface used by saqmaodv
    for (auto j = m_socketAddresses.begin(); j != m_socketAddresses.end(); ++j)
    {
        ...
        TypeHeader tHeader(SAQMAODVTYPE_RREQ);
...
{
    socket->SendTo(packet, 0, InetSocketAddress(destination, SAQMAODV_PORT));
}
...
void
RoutingProtocol::RecvSaqmaodv(Ptr<Socket> socket)
{
    ...
    NS_LOG_DEBUG("SAQMAODV node " << this << " received a SAQMAODV packet from " << sender << " to "
                              << receiver);
    ...
    if (!tHeader.IsValid())
    {
        NS_LOG_DEBUG("SAQMAODV message " << packet->GetUid() << " with unknown type received: "
                                     << tHeader.Get() << ". Drop");
        return; // drop
    }
    switch (tHeader.Get())
    {
    case SAQMAODVTYPE_RREQ: {
        RecvRequest(packet, receiver, sender);
        break;
    }
    case SAQMAODVTYPE_RREP: {
        RecvReply(packet, receiver, sender);
        break;
    }
    case SAQMAODVTYPE_RERR: {
        RecvError(packet, sender);
        break;
    }
    case SAQMAODVTYPE_RREP_ACK: {
        RecvReplyAck(sender);
        break;
    }
    }
...
RoutingProtocol::SendReply(const RreqHeader& rreqHeader, const RoutingTableEntry& toOrigin)
{
    ...
    TypeHeader tHeader(SAQMAODVTYPE_RREP);
    packet->AddHeader(tHeader);
    Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(toOrigin.GetNextHop(), SAQMAODV_PORT));
}
                                             ...
                                             bool gratRep)
{
    ...
    if (gratRep)
    {
        ...
        TypeHeader type(SAQMAODVTYPE_RREP);
        packetToDst->AddHeader(type);
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toDst.GetInterface());
        NS_ASSERT(socket);
        NS_LOG_LOGIC("Send gratuitous RREP " << packet->GetUid());
        socket->SendTo(packetToDst, 0, InetSocketAddress(toDst.GetNextHop(), SAQMAODV_PORT));
    }
}
...
RoutingProtocol::SendReplyAck(Ipv4Address neighbor)
{
    ...
    RrepAckHeader h;
    TypeHeader typeHeader(SAQMAODVTYPE_RREP_ACK);
    Ptr<Packet> packet = Create<Packet>();
    SocketIpTtlTag tag;
    tag.SetTtl(1);
    packet->AddPacketTag(tag);
    ...
    NS_ASSERT(socket);
    socket->SendTo(packet, 0, InetSocketAddress(neighbor, SAQMAODV_PORT));
}
...
RoutingProtocol::RecvError(Ptr<Packet> p, Ipv4Address src)
{
    ...
    for (auto i = unreachable.begin(); i != unreachable.end();)
    {
        ...
        {
            TypeHeader typeHeader(SAQMAODVTYPE_RERR);
            Ptr<Packet> packet = Create<Packet>();
            SocketIpTtlTag tag;
            tag.SetTtl(1);
            packet->AddPacketTag(tag);
                                              ...
                                              Ipv4Address origin)
{
    ...
    packet->AddPacketTag(tag);
    packet->AddHeader(rerrHeader);
    packet->AddHeader(TypeHeader(SAQMAODVTYPE_RERR));
    if (m_routingTable.LookupValidRoute(origin, toOrigin))
    {
        Ptr<Socket> socket = FindSocketWithInterfaceAddress(toOrigin.GetInterface());
        NS_ASSERT(socket);
    ...
    else
    {
        for (auto i = m_socketAddresses.begin(); i != m_socketAddresses.end(); ++i)
        {
            ...
            socket->SendTo(packet->Copy(), 0, InetSocketAddress(destination, SAQMAODV_PORT));
        }
    }
}
...
RoutingProtocol::GetEnergyFraction() const
{
  ...
  if (!node) return 1.0;
  Ptr<ns3::EnergySourceContainer> esc =
      node->GetObject<ns3::EnergySourceContainer>();
  if (!esc || esc->GetN() == 0) return 1.0;
  Ptr<ns3::BasicEnergySource> src =
      DynamicCast<ns3::BasicEnergySource>(esc->Get(0));
  if (!src) return 1.0;
...
RoutingProtocol::PeriodicAdaptiveTick()
{
  ...
  m_qtable.RecomputeSmoothEnergyWeights(GetEnergyFraction());
  // Re-arm the timer
  m_periodicAdaptEvent =
      Simulator::Schedule(m_periodicAdaptInterval,
                          &RoutingProtocol::PeriodicAdaptiveTick, this);
