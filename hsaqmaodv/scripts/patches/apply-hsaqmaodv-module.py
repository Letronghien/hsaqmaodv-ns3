...
#
# hsaqmaodv extends saqmaodv::QTable. saqmaodv-qtable.h is a custom file
# (not part of NS-3 stock), so its directory must be explicitly added to the
# include path — it is not automatically propagated via LIBRARIES_TO_LINK.
...
# saqmaodv-qtable.h is a custom addition not exported by libsaqmaodv's
# default include paths. Add it explicitly so the compiler can find it.
target_include_directories(libhsaqmaodv-obj PUBLIC
    ${CMAKE_SOURCE_DIR}/src/saqmaodv/model
)
"""
